...
class JasaMontir_M extends CI_Model {
    ...
    public function getByMonth($month){
        $this->db->select('a.id as orderid, a.userid, a.nohp, a.nama, a.alamatlengkap, idmerk, merk, type, kendala, resi, idrekening, namabank, norek, namaakun, orderdate, status');
        $this->db->from('montirorders a');
        $this->db->join('motor c', 'a.idmerk = c.id');
        $this->db->join('rekening d', 'a.idrekening = d.id');
